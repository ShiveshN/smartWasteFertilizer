...
def predcrop1():
    if request.method == 'POST':
        ...
        # Validate and classify input data
        sensor_values = [data1, data2, data3, data4]
        if all(0 <= value <= 1023 for value in sensor_values):
            prediction = rf.predict([sensor_values])[0]
            classification_result = "Yes" if prediction == 1 else "No"
            print(f"\nFertilizer Converted: {classification_result}")
            
            # Provide suggestions based on sensor values
            suggestions = []
            if data1 < 400:
                suggestions.append("Oxygen levels are low, consider adding oxygen-related fertilization.")
            if data2 < 400:
                suggestions.append("Methane content is low, consider boosting methane levels.")
            if data3 < 400:
                suggestions.append("Carbon dioxide levels are low, consider increasing CO2 content.")
            if data4 < 400:
                suggestions.append("Hydrogen levels are low, consider hydrogen-based fertilization.")
            
            # Combine suggestions into a single message
            suggestion_message = " ".join(suggestions) if suggestions else "All sensor levels are sufficient."
        else:
            classification_result = "Invalid Input - Sensor values out of range"
            suggestion_message = "No suggestions as input values are invalid."
            print(f"\n{classification_result}")

        # Render the result and suggestions on the template
        return render_template(
            'resultpred.html',
            prediction=classification_result,
            prediction1=classification_result,
